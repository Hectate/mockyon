...
Mockyon is a pseudo-server for Beyond All Reason clients, specifically using [Tachyon Protocol](https://github.com/beyond-all-reason/tachyon) and [Recoil-Autohost](https://github.com/beyond-all-reason/recoil-autohost). It's primary purpose is to serve as a testing ground for client and autohost connections without requiring a fully developed and configured [Teiserver](https://github.com/beyond-all-reason/teiserver) instance. This is primarily accomplished by only retaining state in-memory (there is zero persistence) and not fully implementing most features.

While all requests, responses, and events will conform to protocol, most will not be implemented. Note that the server will auto-succeed for things like `messaging/subscribeReceived` for a smooth client experience, even though messages will not be sent or received.
...
2. Open the static pages in a browser:
    - Client login/auth: `http://<server-host>:8080/login/` (not useful on its own)
...
3. Connect clients (local or remote) to the mock Tachyon websocket endpoint at `ws://<server-host>:8080/tachyon`.
    - If you are connecting from the same machine, `ws://localhost:8080/tachyon` will work in the client's server settings.
    - If you connecting from a different client on the LAN, you may use the IP address in the server log; `ws://192.168.1.10/tachyon` for example.
...
5. Download an engine from the Admin page by entering a specific version string (e.g. `2026.07.04`) and pressing the **Download Engine** button.
